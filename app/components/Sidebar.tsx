...
import { Home, BookOpen, Users, Settings, Mic, BarChart, Layers, ArrowLeft, LogOut, Menu, X } from "lucide-react";
  ...
  return (
    <>
      ...
      <button
        ...
        className="lg:hidden fixed top-4 left-4 z-50 p-2.5 bg-white rounded-xl border border-gray-200 shadow-sm"
      >
        <Menu className="w-5 h-5 text-gray-600" />
      </button>

      {/* Desktop Sidebar */}
      <aside className="hidden lg:flex w-64 border-r border-gray-100 bg-white h-screen fixed left-0 top-0 flex-col shadow-sm z-40">
        {navContent}
      </aside>
      ...
      {open && (
        <>
          ...
          />
          <aside
            className="lg:hidden fixed top-0 left-0 h-full w-72 bg-white border-r border-gray-200 z-50 flex flex-col transition-transform duration-300 translate-x-0"
          >
            {navContent}
          </aside>
        </>
