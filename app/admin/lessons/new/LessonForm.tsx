...
import { Plus, Trash2, Mic, Type, Image as ImageIcon, Save, ArrowLeft, Volume2 } from "lucide-react";
...
export default function LessonForm({ units }: LessonFormProps) {
  const [exercises, setExercises] = useState([
    { id: 1, type: "translation", question: "", answer: "", options: [] }
  ]);

  const addExercise = () => {
    setExercises([...exercises, { id: Date.now(), type: "translation", question: "", answer: "", options: [] }]);
  };
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
              ...
              <select className="w-full p-3 rounded-xl border-2 border-gray-200 focus:border-brand-green focus:outline-none font-medium">
                {units.map((unit) => (
                  <option key={unit.id} value={unit.id}>Unité {unit.order}: {unit.title}</option>
                ))}
              </select>
            </div>
          </div>
